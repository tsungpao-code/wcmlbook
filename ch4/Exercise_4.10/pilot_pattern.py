...
class PilotPattern():
    ...
    def __init__(self, mask, pilots, trainable=False, normalize=False,
        ...
        self._pilots = torch.tensor(pilots)
    ...
    def pilots(self):
        ...
        def norm_pilots():
            ...
            scale = 1/torch.sqrt(torch.mean(scale, axis=-1, keepdims=True))
    ...
    def __init__(self,
        ...
        for i in range(num_tx):
            for j in range(num_streams_per_tx):
                ...
                p = QAMSource([1,1,num_pilot_symbols,num_pilots_per_symbol])
...
def QAMSource(shape):
    random_int = torch.randint(low=0, high=2, size=shape)
    real = torch.where(random_int == 0, -1, 1).float()
    random_int_1 = torch.randint(low=0, high=2, size=shape)
    image = torch.where(random_int_1 == 0, -1, 1).float()
    p = torch.complex(real,image)
    return p