...
if __name__ == "__main__":
    ...
    # 输入图片数据集根父路径
    input_base_path = 'D:/paper coding/ADJSCC(image reconstruction)/ADJSCC_Task/data2/test'

    # 输入的图片格式(后缀名，区分大小写)
    input_fmt = 'jpg'

    # 输出JPEG图片数据集根父路径（未进行LDPC+QAM+AWGN）
    output_base_path = 'D:/paper coding/ADJSCC(image reconstruction)/ADJSCC_Task/data2/test_jpeg'

    # 图片对应字节比特流txt的目录
    output_txt_path = 'D:/paper coding/ADJSCC(image reconstruction)/ADJSCC_Task/data2/test_jpeg_bit'

    # 输出通过了信道传输的JPEG图片数据集根父路径（经过了LDPC+QAM+AWGN）
    channelcoded_output_base_path = 'D:/paper coding/ADJSCC(image reconstruction)/ADJSCC_Task/data2/test_jpeg_with_ldpc_qam_awgn'
    ...
    for img_dir in input_images:
        ...
        if not os.path.exists(directory):
            os.makedirs(directory)
        file_dir = output_txt_path + '/' + test_img_name + '.txt'
        print(img_dir)
