...
def main():
    ...
    #t.train()
    results = t.train()
    df = pd.DataFrame(columns=['test_acc', 'train_acc', 'train_loss'])
    df['test_acc'] = results[0]
    df['train_acc'] = results[1]
    df['train_loss'] = results[2]
    df.to_csv("./save/{}_{}_{}_{}_{}_{}.csv".format(options['optimizer'], options['dataset'], options['model'], options['mu'], options['drop_percent'],options['clients_per_round']))
